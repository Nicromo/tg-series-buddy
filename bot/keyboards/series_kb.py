...
def card_keyboard(
    ...
    *,
    ...
    # Подписка на новые сезоны / премьеру
    notify_label = "🔕 Не следить" if notify_releases else "🔔 Уведомлять о выходе"
    rows.append(
        [InlineKeyboardButton(text=notify_label, callback_data=f"notify:{series_id}")]
    )
